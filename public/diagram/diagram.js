...
function objectsPositionChanged() {
  const newTitle = objectsKeepTheirPosition()
    ? 'ON -> objects will keep their positions (better performance when you have > 100 objects)'
    : 'OFF -> objects will be relocated to fit into the graph layout (graph is easier to read)'
  document
    .getElementById('toggle-pin')
    .setAttribute('title', `Fix objects position: ${newTitle}`)
}
...
function reloadDiagram(elements) {
  ...
  const newElements = elements.filter(
    (element) => !cy.hasElementWithId(element.data.id),
  )
  if (newElements.length) {
    const shouldUpdateLayout =
      !objectsKeepTheirPosition() || cy.elements().length === 0
...
function readyForLayoutElems(elements) {
  const isInElems = (elem) =>
    elements.some((element) => element.id() === elem.id())

  return elements.filter(
    (element) =>
      element.isNode() ||
      (element.isEdge() &&
        isInElems(element.target()) &&
        isInElems(element.source())),
  )
}
...
function changeElementsMode() {
  currentElements.forEach((element) => {
    element.data.mode = isDarkMode() ? 'dark' : 'light'
  })
}
