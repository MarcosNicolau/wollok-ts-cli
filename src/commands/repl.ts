...
export default async function (
  ...
  const autoImportName =
    autoImportPath && replNode(interpreter.evaluation.environment).name
  const repl = Repl({
    input: process.stdin,
    ...
    logger.info(
      successDescription(
        'Dynamic diagram available at: ' +
          bold(`http://localhost:${options.port}`),
      ),
    )
    repl.prompt()
  })
  ...
  try {
    ...
    if (!skipValidations) {
      ...
      if (!problems.length)
        ...
        throw problems.find((_) => _.level === 'error')
    }
    ...
    if (autoImportPath) {
      ...
      // Register the auto-imported package as REPL package
      else
        log(
          failureDescription(
            `File ${valueDescription(
              autoImportPath,
            )} doesn't exist or is outside of project!`,
          ),
        )
    } else {
...
export function interprete(interpreter: Interpreter, line: string): string {
  try {
    const sentenceOrImport = parse.Import.or(parse.Variable)
      .or(parse.Assignment)
      .or(parse.Expression)
      .tryParse(line)
    const error = [sentenceOrImport, ...sentenceOrImport.descendants]
      .flatMap((_) => _.problems ?? [])
      .find((_) => _.level === 'error')
    if (error) throw error

    if (sentenceOrImport.is(Sentence)) {
      const linkedSentence = newSentence(
        sentenceOrImport,
        interpreter.evaluation.environment,
      )
      const unlinkedNode = [linkedSentence, ...linkedSentence.descendants].find(
        (_) => _.is(Reference) && !_.target,
      )
        ...
        if (unlinkedNode.is(Reference)) {
          if (!interpreter.evaluation.currentFrame.get(unlinkedNode.name))
            return failureDescription(
              `Unknown reference ${valueDescription(unlinkedNode.name)}`,
            )
        } else
          return failureDescription(
            `Unknown reference at ${unlinkedNode.sourceInfo}`,
          )
      }
    ...
    if (sentenceOrImport.is(Import)) {
      if (
        !interpreter.evaluation.environment.getNodeOrUndefinedByFQN(
          sentenceOrImport.entity.name,
        )
      )
        throw new Error(
    ...
    return error.type === 'ParsimmonError'
      ? failureDescription(
          `Syntax error:\n${error.message
            .split('\n')
            .filter(notEmpty)
      ...
      : error instanceof WollokException
      ? failureDescription('Evaluation Error!', error)
      : error instanceof ParseError
      ? failureDescription(
          `Syntax Error at offset ${error.sourceMap.start.offset}: ${line.slice(
            error.sourceMap.start.offset,
            error.sourceMap.end.offset,
          )}`,
        )
...
async function initializeClient(options: Options) {
  ...
  server.addListener(
    'error',
    ({ port, code }: { port: string; code: string }) => {
      console.info('')
      if (code === 'EADDRINUSE') {
        console.info(
          yellow(
            bold(
              `⚡ We couldn't start dynamic diagram at port ${port}, because it is already in use. ⚡`,
            ),
          ),
        )
        ...
        console.info(
          yellow(
            bold(
              `⚡ REPL couldn't be started at port ${port}, error code ["${code}]. ⚡`,
            ),
          ),
        )
      }
...
function newSentence<S extends Sentence>(
  sentence: S,
...
export const replNode = (environment: Environment): Package =>
  environment.getNodeByFQN<Package>(REPL)
