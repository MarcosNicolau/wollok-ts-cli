...
export default async function (
  ...
  logger.info(
    `Running all tests ${
      filter ? `matching ${valueDescription(filter)} ` : ''
    }on ${valueDescription(project)}`,
  )
  ...
  const targets = environment.descendants
    .filter(is(Test))
    .filter(
      (test) =>
    ...
    match(node)(
      when(Test)((node) => {
        if (targets.includes(node)) {
          const tabulation = '  '.repeat(
            node.fullyQualifiedName.split('.').length - 1,
          )
          try {
