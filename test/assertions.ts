...
declare global {
  export namespace Chai {
    interface Assertion {
      // TODO: split into the separate modules
      connect: (
        label: string,
        sourceLabel: string,
        targetLabel: string,
...
export const diagramAssertions: Chai.ChaiPlugin = (chai) => {
  ...
  Assertion.addMethod('nodeWith', function (query: ElementDefinitionQuery) {
    ...
    const diagram = this._obj.map(({ data }: any) =>
      Object.fromEntries(
        Object.entries(data).filter(([key]) =>
          Object.keys(query).includes(key),
        ),
      ),
    )
  ...
  Assertion.addMethod(
    'connect',
    function (label: string, source: string, target: string) {
      const {
        data: { id: sourceId },
      } = this._obj.find(({ data }: any) => data.label === source)
      const {
        data: { id: targetId },
      } = this._obj.find(({ data }: any) => data.label === target)
      new Assertion(this._obj).to.include.nodeWith({
        label,
        source: sourceId,
        target: targetId,
      })
